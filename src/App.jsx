...
function stateToParams(state, lang, sendIdx) {
  const p = new URLSearchParams();
  p.set("lang", lang);
  p.set("sendIdx", sendIdx);
...
export default function App() {
  ...
  const [sendIdx, setSendIdx] = useState(() => {
    const p = new URLSearchParams(window.location.search);
    const idx = parseInt(p.get("sendIdx") || "0", 10);
    return idx >= 0 && idx < ALL_SENDS.length ? idx : 0;
  });
  ...
  // Sync URL
  useEffect(() => {
    window.history.replaceState(null, "", `${window.location.pathname}?${stateToParams(vars, lang, sendIdx)}`);
  }, [vars, lang, sendIdx]);
  ...
  const copyLink = () => {
    const url = `${window.location.origin}${window.location.pathname}?${stateToParams(vars, lang, sendIdx)}`;
    navigator.clipboard.writeText(url).then(() => showToast("✓ Link copiado al portapapeles"));
  };

  // Template activo según send seleccionado
  const currentTemplates = ALL_SENDS[sendIdx]?.templates || {};
  const iframeDoc = buildIframeDoc(vars, currentTemplates[lang] || "");
    ...
    <div style={{ minHeight: "100vh", background: "#13102a", fontFamily: "'Segoe UI', sans-serif" }}>
          ...
          <div style={{ padding: "11px 0 13px", display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            <div style={{
              ...
              <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                {ALL_SENDS.map((send, idx) => (
                  <OptionBtn
                    key={send.key}
                    active={sendIdx === idx}
                    onClick={() => setSendIdx(idx)}
                    color="#a78bfa"
                  >{send.label}</OptionBtn>
                ))}
              </div>
            </div>
            ...
            <div style={{
              ...
              <span style={{
                color: "#53D989", fontSize: 11, fontWeight: 700,
                textTransform: "uppercase", letterSpacing: 0.8, minWidth: 145,
              }}>🌍 Country / Lang</span>
              <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "24px 20px 60px" }}>
        ...
        <p style={{ textAlign: "center", color: "rgba(255,255,255,0.25)", fontSize: 12, marginTop: 14 }}>
          Visualizando: <strong>{ALL_SENDS[sendIdx]?.label}</strong> · <strong>{lang.toUpperCase()}</strong> · Zoom: {Math.round(zoom * 100)}%
        </p>
      </div>
